...
def extract_tasks(lines: list[str], line_no: int) -> Tuple[str, str, str, str]:
    '''
...
class StdinLineSource:
    ...
    def get_tasks(self) -> Iterable[Task]:
        ...
        for line_no, line in enumerate(self.stream, start=1):
            ...
            if not line.strip():
                continue
            id, title, author, content = extract_tasks(lines, line_no)
            yield Task(id=id, title=title, author=author, content=content)
